...
int lkmdbg_runtime_hook_set_owner_proc_hidden(pid_t owner_tgid, bool hidden)
{
	...
	/*
	 * Let any pre-hide procfs lookups/readdir callbacks drain before we
	 * return to user space, then invalidate cached dentries again under the
	 * now-active hidden policy.
	 */
	synchronize_rcu();
