...
- Build user-space helpers with:
  ...
  - `tools/examples/*.c` are one-feature-per-file runnable examples. New and migrated examples should use `liblkmdbg`; legacy examples remain buildable with the bridge C sources during migration.
...
- Do not move general debugger policy into `/proc/version` when the session fd transport already models it.
